...
pub const Topic = struct {
    ...
    pub fn addNewCGroup(self: *Self, io: Io, gpa: Allocator, cgroup_id: u32) !void {
        try self.mq_lock.lock(io); // Block until acquire
        defer self.mq_lock.unlock(io); // Release on exit
        self.topic_lock.lock(); // Block cgroup for adding
        defer self.topic_lock.unlock(); // Release on exit
        try self.cgroups.append(gpa, try CGroup.new(gpa, cgroup_id, self.topic_id, self.mq.pop_num));
    ...
    pub fn addMessage(self: *Self, io: Io, gpa: Allocator, message: *const message_util.ProduceConsumeMessage) !void {
        ...
        while (!self.mq.push_back(m_copy)) {
            ...
            self.mq_lock.unlock(io); // Release
            // This case will not need a broadcast, since it's a queue full case.
    ...
    pub fn tryPopMessage(self: *Self, io: Io, gpa: Allocator) !void {
        while (!self.is_terminated) {
            ...
            try self.mq_lock.lock(io); // Lock to pop
            if (min_offset == 1000000000) {
                self.mq_lock.unlock(io);
            ...
            defer self.mq_lock.unlock(io);
            self.topic_lock.unlockShared(); // Unlock on done loop
        }
        // Once terminated, free all remaining messages
        std.debug.print("Terminated pop for topic {}\n", .{self.topic_id});
